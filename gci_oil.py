...
parser.add_argument('--query', default='oil', type=str,
...
def handle_args(args):
	if args.query == 'oil':
		query_select = ['oil price','gas price']
	...
	suffix = '_' + ratio_str + '_' + args.query + '_' + str(args.seed)
	return query_select, num_select1, num_select2, num_clusters, ratio_str, suffix
...
def load_data(addr, args):
	...
	random_idx = np.arange(len(query_clean))
	np.random.shuffle(random_idx)
	fact_clean = np.array(fact_clean)[random_idx]
	query_clean = np.array(query_clean)[random_idx]
	fact_original_new = []
	for i in random_idx:
	...
	return fact_clean, query_clean, fact_original, random_idx, train_split, wv

def extract_keywords(addr, fact_original, query_clean, train_split, num_select1=20, num_select2=5):
	import pke
	import spacy
	from yake_modified_en import YAKE
	...
	def count_df(fact_accu, stop_words):
		def count_df_accu(text, stop_words):
			...
			extractor.load_document(stop_words, "".join(text), language='en', spacy_model=nlp)
	...
	accu_train = query_clean[:train_split]
	accu_dict = {}
	fact_accu = []
	id2accu = {}
	...
	stop_word_file = addr + "ENstopwords.txt"
...
def cluster_keywords(wv, original_combine_key, num_clusters=10):
...
accu_select, num_select1, num_select2, num_clusters, ratio_str, suffix = handle_args(
	args)
graph_samples_num = 5
fact_clean, query_clean, fact_original, random_idx, train_split, wv = load_data(
	addr, args)
if not args.restart and not args.sensitivity:
	original_combine_key = \
		extract_keywords(addr, fact_original, query_clean,
